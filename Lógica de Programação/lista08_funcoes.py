...
def lucky_sum(a, b, c):
  numeros = [a, b, c]
  if 13 in numeros:
    indice = numeros.index(13)
    numeros = sum(numeros[:indice])
  else: numeros = sum(numeros)
  return numeros
...
def end_other(a, b):
  minusculaa = a.lower()
  minusculab = b.lower()
  return minusculaa.endswith(minusculab) or minusculab.endswith(minusculaa)
...
def fila_tijolos(n_peq, n_gra, meta):
  if (meta % 1 == 0 and meta // 1 <= n_peq) or (meta % 5 == 0 and meta // 5 <= n_gra):
    return True
  qtde_n_gra = meta // 5
  if qtde_n_gra > n_gra or n_gra == 0:
    qtde_n_gra = n_gra
  meta -= qtde_n_gra * 5
  qtde_n_peq = meta // 1
  if qtde_n_peq > n_peq or n_peq == 0:
    qtde_n_peq = n_peq
  meta -= qtde_n_peq * 1
  return meta == 0
