...
def label_grid(master, text, tamanho, column, row, padx, pady, sticky):
...
def criar_escala(master, row, padx, pady):
    escala = Scale(master=master, from_=1, to=5, length=500, tickinterval=1, orient=HORIZONTAL, bg='#fae8e8', font='Calibre, 10', highlightcolor='#c5a8b0', troughcolor='#c5a8b0')
    escala.grid(column=0, row=row, padx=padx, pady=pady, sticky='w')
    def resposta():
        if escala.get() <= 3:
            label_grid(frm_avaliacao, 'Feedback obrigatório: ', 13, 1, row, 2, 0, 'e')
            criar_entrada(frm_avaliacao, row, 2, 1, 0, 'w')
    nome=Button(master=frm_avaliacao, text='Enviar', fg='#1a1d1a', bg='#d9d9d9', font=('Calibre', 15), width=10, height=1, activebackground='#c5a8b0', command=resposta)
...
label_grid(frm_geral, 'Autoavaliação', 30, 0, 0, 30, 30, 'w')
label_grid(frm_geral, 'Nome do usuário', 20, 0, 1, 30, 0, 'w')  
label_grid(frm_geral, 'Prazo para realizar a autoavaliação da {nº da Sprint}', 15, 0, 2, 30, 20, 'w')
label_grid(frm_geral, 'Breve explicação sobre a avaliação: Falar sobre a escala Likert e dos feedbacks obrigatórios para notas abaixo ou iguais a 3', 15, 0, 3, 30, 30, 'w')  

# Textos das questões da avaliação
label_grid(frm_avaliacao, 'Primeira questão', 15, 0, 4, 30, 5, 'w')
label_grid(frm_avaliacao, 'Segunda questão', 15, 0, 7, 30, 5, 'w')
label_grid(frm_avaliacao, 'Terceira questão', 15, 0, 10, 30, 5, 'w')
label_grid(frm_avaliacao, 'Quarta questão', 15, 0, 13, 30, 5, 'w') 
label_grid(frm_avaliacao, 'Quinta questão', 15, 0, 16, 30, 5, 'w')
label_grid(frm_avaliacao, 'Sexta questão', 15, 0, 19, 30, 5, 'w')
label_grid(frm_avaliacao, 'Sétima questão', 15, 0, 22, 30, 5, 'w')  
label_grid(frm_avaliacao, 'Oitava questão', 15, 0, 25, 30, 5, 'w')

# Descrição da tabela
label_grid(frm_avaliacao, 'Péssimo (1)           Ruim (2)              Regular (3)                Bom (4)               Ótimo (5)', 10, 0, 5, 30, 0, 'w')
label_grid(frm_avaliacao, 'Péssimo (1)           Ruim (2)              Regular (3)                Bom (4)               Ótimo (5)', 10, 0, 8, 30, 0, 'w')
label_grid(frm_avaliacao, 'Péssimo (1)           Ruim (2)              Regular (3)                Bom (4)               Ótimo (5)', 10, 0, 11, 30, 0, 'w')
label_grid(frm_avaliacao, 'Péssimo (1)           Ruim (2)              Regular (3)                Bom (4)               Ótimo (5)', 10, 0, 14, 30, 0, 'w') 
label_grid(frm_avaliacao, 'Péssimo (1)           Ruim (2)              Regular (3)                Bom (4)               Ótimo (5)', 10, 0, 17, 30, 0, 'w')
label_grid(frm_avaliacao, 'Péssimo (1)           Ruim (2)              Regular (3)                Bom (4)               Ótimo (5)', 10, 0, 20, 30, 0, 'w')
label_grid(frm_avaliacao, 'Péssimo (1)           Ruim (2)              Regular (3)                Bom (4)               Ótimo (5)', 10, 0, 23, 30, 0, 'w')  
label_grid(frm_avaliacao, 'Péssimo (1)           Ruim (2)              Regular (3)                Bom (4)               Ótimo (5)', 10, 0, 26, 30, 0, 'w')
